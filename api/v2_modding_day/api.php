...
function getMPKeyword($mp, $keyword) {
        $query = "SELECT * from SentimentDemo where MP='" . $mp . "' and keyword='".$keyword."'";
	...
	$i=0;
        while($row = mysql_fetch_array( $result )) {
                $answer['filename'] = $row[0]; // this is a push operation. PHP!
                $answer['MP'] = $row[1]; // this is a push operation. PHP!
                $answer['keyword'] = $row[2]; // this is a push operation. PHP!
                $answer['content'] = $row[3]; // this is a push operation. PHP!
                $answerArr[$i] = $answer;
		$i++;
	}

        return json_encode($answerArr);
}
